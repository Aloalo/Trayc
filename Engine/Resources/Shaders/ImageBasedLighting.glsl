...
vec3 IBL(in vec3 V, in vec3 N, in float roughness, in vec3 Fdiff, in vec3 albedo)
{
    ...
    vec3 specular = textureLod(reflectionMap, R, lod * roughness).rgb;
    
    return mix(albedo * irradiance, specular, Fdiff);
}
